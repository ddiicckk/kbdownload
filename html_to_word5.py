...
Columns required:
    number  -> file name base
    version -> appended to filename
    text    -> HTML content

Output filename:
    {number}_{version}.docx

HTML is converted to REAL Word content (no altChunk).
Supports: headings, paragraphs, lists, tables, bold/italic/underline, br, links.
"""
...
# CONFIGURATION
# ==========================
CSV_PATH = "kb_articles.csv"      # <-- change if needed
COL_ID = "number"
COL_VERSION = "version"
COL_HTML = "text"
...
class HtmlToDocx:
    ...
    def _append_inline(self, paragraph, node, fmt=None):
        ...
        for child in node.children:
            self._append_inline(paragraph, child, new_fmt)
...
def main():
    ...
    missing = [c for c in (COL_ID, COL_VERSION, COL_HTML) if c not in df.columns]
    if missing:
    ...
    for idx, row in df.iterrows():
        num = row.get(COL_ID)
        ver = row.get(COL_VERSION)
        html = row.get(COL_HTML)

        if pd.isna(num) or pd.isna(ver) or pd.isna(html) or str(html).strip() == "":
            skipped += 1
            with open(LOG_PATH, "a", encoding="utf-8") as log:
                log.write(f"[SKIP] Row {idx+1}: missing number/version/text\n")
            continue

        # filename: number_version
        filename_base = sanitize_filename(f"{num}_{ver}")
        out_path = ensure_unique_path(OUTPUT_DIR, filename_base, ".docx")
        ...
        try:
            ...
            if not doc.paragraphs and not doc.tables:
                doc.add_paragraph("(No usable content)")

            doc.save(out_path)
            exported += 1

            with open(LOG_PATH, "a", encoding="utf-8") as log:
                log.write(f"[OK] {num}_{ver} -> {out_path}\n")

        except Exception as e:
            failures += 1
            with open(LOG_PATH, "a", encoding="utf-8") as log:
                log.write(f"[FAIL] Row {idx+1} '{num}_{ver}': {e}\n")
